"""Training entry point for exp001_baseline."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a lightweight BirdCLEF baseline.")
    parser.add_argument(
        "--metadata",
        type=Path,
        default=DEFAULT_METADATA,
        help="Metadata CSV to train from. Falls back to train.csv if missing.",
    )
    parser.add_argument("--limit", type=int, default=None, help="Optional row limit for quick smoke runs.")
    parser.add_argument("--sample-rate", type=int, default=32000, help="Target sample rate.")
    parser.add_argument("--duration", type=float, default=5.0, help="Seconds to use from each training clip.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Drop classes with fewer than this many rows before splitting.",
    )
...
def extract_features(path: Path, sample_rate: int, duration: float, n_mels: int) -> np.ndarray:
...
def build_feature_matrix(df: pd.DataFrame, sample_rate: int, duration: float, n_mels: int) -> np.ndarray:
    features: list[np.ndarray] = []
    for filename in tqdm(df["filename"], desc="Extracting features"):
...
def main() -> None:
    ...
    joblib.dump(model_payload, ARTIFACT_DIR / "baseline_model.joblib")

    with open(ARTIFACT_DIR / "metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    print(json.dumps(metrics, indent=2))
    print(f"Saved model to {ARTIFACT_DIR / 'baseline_model.joblib'}")
